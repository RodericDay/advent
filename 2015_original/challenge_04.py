...
def transform(key, pad):
    string = key+str(pad)
    hashed = hashlib.md5(string.encode()).digest()
    readable = ''.join("{:0>2x}".format(c) for c in hashed)
    return readable

def find(key, n):
    for pad in itertools.count():
        readable = transform(key, pad)
        if readable.startswith('0'*n):
            return pad, readable

# print(find('abcdef'))
# print(find('pqrstuv'))
# print(find('yzbqklnj', 5)) # 4.1s
# print(find('yzbqklnj', 6)) # much longer
